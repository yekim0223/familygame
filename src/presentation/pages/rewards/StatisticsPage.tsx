...
export default function StatisticsPage() {
  ...
  return (
    <div className="p-3 pb-4 space-y-3">
      <h1 className="font-pixel text-xs text-gold">📊 통계</h1>

      {/* 이번 달 요약 */}
      <PixelCard padding="sm">
        <p className="font-pixel text-xs text-purple mb-2">이번 달 요약</p>
        <div className="grid grid-cols-3 gap-2 text-center">
          <div>
            <p className="font-korean text-xl">📋</p>
            <p className="font-pixel text-xs text-pixel-dark">{thisMonthMissions.length}</p>
            <p className="font-korean text-xs text-stone">총 미션</p>
          </div>
          <div>
            <p className="font-korean text-xl">✅</p>
            <p className="font-pixel text-xs text-pixel-dark">{approvedCount}</p>
            <p className="font-korean text-xs text-stone">승인됨</p>
          </div>
          <div>
            <p className="font-korean text-xl">💰</p>
            <p className="font-pixel text-xs text-pixel-dark">{totalMoney.toLocaleString()}원</p>
            <p className="font-korean text-xs text-stone">용돈</p>
          </div>
        </div>
      </PixelCard>
      ...
      <PixelCard padding="sm">
        <div className="flex items-center justify-between mb-1">
          <p className="font-pixel text-xs text-purple">미션 달성률</p>
      ...
      {/* 6개월 보상 트렌드 */}
      <PixelCard padding="sm">
        <p className="font-pixel text-xs text-purple mb-3">📈 최근 6개월 용돈 트렌드</p>
        <PixelBarChart data={trendData} unit="원" height={100} />
      </PixelCard>

      {/* 게임시간 통계 */}
      <PixelCard padding="sm">
        <p className="font-pixel text-xs text-purple mb-2">🎮 이번 달 게임시간</p>
