// Design Ref: §5.3 SCR-03 RegisterPage — 4단계 회원가입
...
// 역할별 재미있는 이모지 — 아빠:마술사 엄마:요정 하윤:천사 서윤:삐에로
...
export default function RegisterPage() {
  ...
  // Step 2 진입 시 이미 가입된 역할 조회 (auth 선확보 후 Firestore 호출)
  useEffect(() => {
    if (step !== 2) return
    const fid = localStorage.getItem('familyId') || authFamilyId || ''
    if (!fid) return
    // localStorage 동기화
    if (fid) localStorage.setItem('familyId', fid)
    startAnonymousSession().then(() =>
      getMembersByFamily(fid).then(({ data }) => {
        setRegisteredRealNames(data.map(m => m.realName))
      })
    )
  }, [step, authFamilyId])
  ...
  const errs = {
    ...
    pinConfirm: w('pinConfirm') && w('pin') !== w('pinConfirm')
                ? 'PIN 번호가 일치하지 않아요' : '',
  ...
  const FAMILY_FULL_MSG = '이미 모든 가족 구성원이 등록되었습니다.'
  ...
  const handleStep2Next = async () => {
    ...
    // 비-DAD: localStorage → authStore 순으로 familyId 조회
    const storedFamilyId = localStorage.getItem('familyId') || authFamilyId || ''
    if (storedFamilyId) {
      localStorage.setItem('familyId', storedFamilyId)  // 동기화
    } else {
    ...
    try {
      // Firestore 호출 전 익명 세션 확보
      const { uid } = await startAnonymousSession()
      if (!uid) {
        setStep2ServerError('서버에 연결할 수 없어요. 잠시 후 다시 시도해주세요.')
        return
      }
      ...
      if (!settings) {
        setStep2ServerError('가족 정보를 찾을 수 없어요. 아빠에게 문의해주세요.')
        return
      }
      const codeHash = await hashFamilyCode(code)
      if (settings.familyCodeHash !== codeHash) {
        setStep2ServerError('가족 인증키가 맞지 않아요.')
        return
      }

      const { data: members } = await getMembersByFamily(storedFamilyId)
      if (members.length >= 4) { setStep2ServerError(FAMILY_FULL_MSG); return }
  ...
  const onSubmit = async (data: FormData) => {
    if (!data.pin || data.pin !== data.pinConfirm) {
      setError('PIN 번호를 다시 확인해주세요')
      return
    }
    setLoading(true)
    setError('')
    ...
    try {
      const result = await signUp({
        ...
        realName,
        familyCode: data.familyCode.trim(),
        // familyId는 signUp.ts에서 localStorage로 자동 조회 (비-DAD)
        familyId: undefined,
      ...
      if (result.error === 'ALREADY_REGISTERED' && result.familyId && result.memberId) {
        ...
        if (loginOk && member) {
          ...
          setStoreFamilyId(result.familyId)
          navigate('/home')
        } else {
          setError('이미 가입된 계정이에요. PIN 번호를 다시 확인해주세요.')
        }
        return
      }
      ...
      newFamilyId = result.familyId
      newMemberId = result.memberId

      // 가입 직후 즉시 authStore 업데이트 (새로고침 없이 로그인 상태 반영)
      const { success: loginOk, member } = await login(newFamilyId, newMemberId, data.pin)
      if (loginOk && member) {
        setCurrentMember(member)
        setStoreFamilyId(newFamilyId)
      }
      ...
      console.error('[SignUp catch]', { code, msg, e })
  ...
  if (successDone) {
    return (
      <div className="min-h-screen bg-minecraft flex flex-col items-center justify-center px-4">
        <PixelCard className="w-full max-w-sm text-center space-y-4">
          <div className="text-6xl">🎉</div>
          <h2 className="font-korean text-lg font-bold text-purple">
            축하합니다. 가입이 완료되었어요
          </h2>
          <p className="font-korean text-sm font-bold text-stone leading-relaxed font-bold">
            이제 엄마·하윤·서윤도 가입할 수 있어요.<br/>
            아래 <span className="text-gold">가족 인증키</span>를 설정해주세요.
          </p>
          <PixelButton type="button" variant="gold" fullWidth size="lg"
    ...
    <div className="min-h-screen bg-minecraft flex flex-col">
      ...
      <div className="flex-1 px-4 pb-8">
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          {/* ── STEP 1: 닉네임 ──────────────────────────────── */}
          {step === 1 && (
            <PixelCard className="mt-2">
              <h2 className="font-korean text-base font-bold text-purple mb-1">
                사용할 이름(닉네임)을 알려줘
              </h2>
              <p className="font-korean text-sm font-bold text-stone mb-4">
                앱에서 보여질 닉네임이에요.<br/>닉네임 옆에 실제 이름이 함께 표시돼요.
              </p>
              <input
                ...
                maxLength={10}
                autoFocus
                className={`w-full bg-pixel-dark text-gold font-korean text-base
                           border-4 px-3 py-3 focus:outline-none focus:border-gold
                           ${errs.nickname && w('nickname') ? 'border-rejected' : 'border-pixel-dark'}`}
              />
              {errs.nickname && w('nickname') && (
                <p className="font-korean text-sm font-bold text-rejected mt-1">{errs.nickname}</p>
              )}
              <p className="font-korean text-sm font-bold text-stone mt-2">최대 10자 · 언제든지 수정 가능</p>
              <PixelButton type="button" variant="gold" fullWidth size="lg" className="mt-4"
          ...
          {/* ── STEP 2: 역할 + 인증키 + PIN ─────────────────── */}
          {step === 2 && (
            <PixelCard className="mt-2 space-y-4">

              {/* 역할 선택 */}
              <div>
                <h2 className="font-korean text-base font-bold text-purple mb-3">역할 선택</h2>
                  ...
                  {SELECTABLE_ROLES.map(role => {
                    ...
                    return (
                      <button
                        ...
                        className={[
                          'flex flex-col items-center py-3 px-2 border-4 font-korean text-sm',
                          'transition-all relative',
                          isRegistered
                            ? 'border-stone bg-stone/20 text-stone cursor-not-allowed opacity-50'
                            : isSelected
                              ? 'border-gold bg-purple text-white'
                              : 'border-pixel-dark bg-cream text-pixel-dark hover:border-purple',
                        ].join(' ')}
                      >
                        <span className="text-3xl mb-1">{ROLE_ICONS[role]}</span>
                        <span className="font-bold font-korean text-sm">{UI_ROLE_LABELS[role]}</span>
                        <span className="text-xs mt-0.5 opacity-80 font-korean font-bold">
              ...
              {/* 가족 인증키 */}
              <div>
                <label className="font-korean text-sm font-bold text-pixel-dark block mb-1">
                  가족 인증키
                </label>
                <div className="relative">
                  <input
                    ...
                    className={`w-full bg-pixel-dark text-gold font-korean text-base
                               border-4 px-3 py-2.5 pr-10 focus:outline-none focus:border-gold
                               ${errs.familyCode && w('familyCode') ? 'border-rejected' : 'border-pixel-dark'}`}
                  />
                ...
                {errs.familyCode && w('familyCode') !== undefined && (
                  <p className="font-korean text-sm font-bold text-rejected mt-1">{errs.familyCode}</p>
                )}
                {watchedRole === 'DAD' && !errs.familyCode && (
                  <p className="font-korean text-sm font-bold text-stone mt-1">
                    기본값: {DEFAULT_FAMILY_CODE} — 마스터 설정에서 변경 가능
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="relative mb-2">
                  <input
                    ...
                    className={`w-full bg-pixel-dark text-gold font-korean text-base
                               border-4 px-3 py-2.5 pr-10 focus:outline-none focus:border-gold
                               ${errs.pin && w('pin') ? 'border-rejected' : 'border-pixel-dark'}`}
                  />
                ...
                {errs.pin && w('pin') && (
                  <p className="font-korean text-sm font-bold text-rejected mb-1">{errs.pin}</p>
                )}
                <div className="relative">
                  <input
                    ...
                    className={`w-full bg-pixel-dark text-gold font-korean text-base
                               border-4 px-3 py-2.5 pr-10 focus:outline-none focus:border-gold
                               ${errs.pinConfirm ? 'border-rejected' : 'border-pixel-dark'}`}
                  />
                ...
                {errs.pinConfirm && (
                  <p className="font-korean text-sm font-bold text-rejected mt-1">{errs.pinConfirm}</p>
                )}
                {!errs.pin && !errs.pinConfirm && w('pin') && w('pinConfirm') && w('pin') === w('pinConfirm') && (
                  <p className="font-korean text-sm font-bold text-approved mt-1">PIN 번호가 일치해요</p>
                )}
              </div>

              {step2ServerError && (
                <div className="bg-rejected/10 border-4 border-rejected px-3 py-3">
                  <p className="font-korean text-sm font-bold text-rejected font-bold">⚠ 가입 불가</p>
                  <p className="font-korean text-xs font-bold text-rejected mt-1">{step2ServerError}</p>
                </div>
              )}
          ...
          {step === 3 && (
            <PixelCard className="mt-2 space-y-4">
              <div>
                <h2 className="font-korean text-base font-bold text-purple mb-1">생년월일</h2>
                <p className="font-korean text-sm font-bold text-stone mb-3">달력에 생일이 자동으로 표시돼요.</p>

                <div className="flex gap-2 mb-4">
                  {(['양력', '음력'] as const).map(ct => (
                    <button key={ct} type="button" onClick={() => setValue('calendarType', ct)}
                      className={`flex-1 py-2.5 font-korean text-sm border-4 border-pixel-dark font-bold
                                  ${watchedCal === ct ? 'bg-purple text-white' : 'bg-cream text-pixel-dark'}`}>
                      {ct}
                    </button>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <div key={f.key}>
                      <label className="font-korean text-xs font-bold text-stone block mb-1">{f.label}</label>
                      <input
                        ...
                        min={f.min} max={f.max}
                        className="w-full bg-pixel-dark text-gold font-korean text-base
                                   border-4 border-pixel-dark px-2 py-2.5
                ...
                {(errs.birthYear || errs.birthMonth || errs.birthDay) && (
                  <div className="mt-2 space-y-0.5">
                    {errs.birthYear  && w('birthYear')  && <p className="font-korean text-sm font-bold text-rejected">{errs.birthYear}</p>}
                    {errs.birthMonth && w('birthMonth') && <p className="font-korean text-sm font-bold text-rejected">{errs.birthMonth}</p>}
                    {errs.birthDay   && w('birthDay')   && <p className="font-korean text-sm font-bold text-rejected">{errs.birthDay}</p>}
                  </div>
                )}
                {watchedCal === '음력' && (
          ...
          {step === 4 && (
            <PixelCard className="mt-2">
              <h2 className="font-korean text-base font-bold text-purple mb-1">내 캐릭터 선택</h2>
              <p className="font-korean text-sm font-bold text-stone mb-3">
                ...
                {(REGISTER_CHARACTERS[watchedRole] ?? []).map(char => {
                  ...
                  return (
                    <button key={char.id} type="button"
                      ...
                      className={`flex items-center gap-2 px-3 py-2.5 border-4 text-left
                                  ${isSelected
                                    ? 'bg-purple text-white border-gold'
                                    : 'bg-cream text-pixel-dark border-pixel-dark hover:bg-yellow-50'}`}
                    >
                      <span className={`text-2xl flex-shrink-0 w-8 h-8 flex items-center justify-center
                                        border-2 ${isSelected ? 'border-gold bg-purple/40' : 'border-pixel-dark bg-cream'}`}>
                        {char.emoji}
                      </span>
                      <span className={`font-korean text-xs leading-tight flex-1 ${isSelected ? 'font-bold' : ''}`}>
                        {char.label}
                      </span>
                      {isSelected && <span className="text-gold font-bold flex-shrink-0">✓</span>}
                    </button>
                  )
                })}
              </div>
              ...
              {error && (
                <div className="bg-rejected/10 border-4 border-rejected px-3 py-3 mb-3 space-y-2">
                  <p className="font-korean text-sm font-bold text-rejected font-bold">⚠ 가입에 실패했어요</p>
                  <p className="font-korean text-xs font-bold text-rejected leading-relaxed">{error}</p>
                  <button type="button" onClick={() => setError('')}
                    className="font-korean text-xs font-bold text-stone underline">닫고 다시 시도하기</button>
                </div>
              )}
              ...
              <p className="font-korean text-sm font-bold text-stone mt-2 text-center">
                캐릭터는 나중에 프로필에서 변경 가능해요
              </p>
            </PixelCard>
          )}
